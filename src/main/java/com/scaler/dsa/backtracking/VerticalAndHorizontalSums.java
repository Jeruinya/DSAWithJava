...
Given a matrix B, of size N x M, you are allowed to do at most A special operations on this grid such that there is 
no vertical or horizontal contiguous subarray that has a sum greater than C.
...
public class VerticalAndHorizontalSums {
	...
	private static boolean check(int u, int v, int op_left, int rsum) {
		if (rsum > sum)
			return false;
		if (rsum < 0)
			rsum = 0;
