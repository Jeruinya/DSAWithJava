...
She decides that every time she will choose a box having the minimum number of candies, eat half of the candies 
and put the remaining candies in the other box that has the minimum number of candies.
...
public class MishaAndCandies {
	...
	public static void main(String[] args) {
		int A[] = { 3, 2, 3 };
		int B = 4;
		System.out.println(findMinimumCandies(A, B));

	}

	public static int findMinimumCandies(int[] A, int B) {
		...
		while (pq.size() != 0) {
			...
			res += candies / 2;
			if (pq.size() > 0) {
				int nextCandies = pq.poll();
				nextCandies += candies - (candies / 2);
				pq.add(nextCandies);
			}
		}
